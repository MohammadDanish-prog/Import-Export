    ...
    <SiteLayout>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12 md:px-10">
        <div className="grid gap-6 lg:grid-cols-2">
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-3xl border border-border bg-gradient-to-br from-brand-soft to-background p-8">
            <Target className="h-9 w-9 text-brand" />
            <h2 className="mt-4 font-display text-2xl font-bold">Our Mission</h2>
            <p className="mt-2 text-muted-foreground">Deliver fresh produce globally with reliability and quality — empowering food businesses with consistent supply, transparent pricing and dependable logistics.</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="rounded-3xl border border-border bg-card p-8">
            <Eye className="h-9 w-9 text-accent-orange" />
            <h2 className="mt-4 font-display text-2xl font-bold">Our Vision</h2>
            <p className="mt-2 text-muted-foreground">Become a trusted international leader in fresh produce trading — recognized for integrity, quality assurance and operational excellence on every shipment.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12 md:px-10">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <motion.div key={v.t} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.06 }} className="rounded-2xl border border-border bg-card p-6 hover-lift">
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10">
        <div className="rounded-3xl bg-gradient-brand p-10 text-brand-foreground md:p-14">
          <h2 className="font-display text-3xl font-bold md:text-4xl">Let's build a long-term trade partnership.</h2>
          <p className="mt-3 max-w-xl text-white/85">Whether you're sourcing a single SKU or stocking a full container — we'll meet your specifications, packaging and timeline.</p>
          <Link to="/contact" className="mt-6 inline-flex rounded-full bg-white px-6 py-3 text-sm font-semibold text-brand shadow-elevated hover-lift">Get in touch</Link>
        </div>
      </section>
    </SiteLayout>
  );
